...
int main(int argc, char* argv[], char* envp[]){
	...
	while((lastPicNr = Fg_getLastPicNumberBlockingEx(fg,lastPicNr+1,nCamPort,10,pMem0))< MaxPics) {
		...
		Fg_getParameterEx(fg, FG_TIMESTAMP, &nTimestamp, nCamPort, pMem0, lastPicNr);
		if(lastPicNr == 1)
			nLastTimestamp = nTimestamp;


		::DrawBuffer(nId,Fg_getImagePtrEx(fg,lastPicNr,0,pMem0),(int)lastPicNr,"");
		fprintf(stdout,"TS: %d: %d - %d microSec \n",(int)lastPicNr,nTimestamp,nTimestamp-nLastTimestamp);
		nLastTimestamp = nTimestamp;
	}

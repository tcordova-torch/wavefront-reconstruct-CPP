...
int main(int argc, char* argv[], char* envp[]){
	...
	int nTapFormat	 = FG_CL_MEDIUM_8_BIT;
	int nUseDval	 = FG_YES;
	int nPixelformat = FG_GRAY16;

	if(Fg_setParameter(fg,FG_CAMERA_LINK_CAMTYP,&nTapFormat,nCamPort)<0){ status = ErrorMessageWait(fg); return status;}

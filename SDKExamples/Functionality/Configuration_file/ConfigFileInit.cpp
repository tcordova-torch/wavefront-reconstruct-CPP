...
int main(int argc, char* argv[], char* envp[]){
	...
	if((status = Fg_saveConfig(fg,CONFIG_FILENAME)) < 0){
		status = ErrorMessageWait(fg);
		return status;
	} else {
		fprintf(stdout,"Config. file saved.");
	}

	// Unload the Dll
	Fg_FreeGrabber(fg);

	// Initialize the grabber with the previous saved Config. file
	if((fg = Fg_InitConfig(CONFIG_FILENAME,nBoard)) == NULL) {
		status = ErrorMessageWait(fg);
		return status;
	}
	fprintf(stdout,"Load Config File ok\n");

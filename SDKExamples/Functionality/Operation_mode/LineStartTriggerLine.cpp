...
int main(int argc, char* argv[], char* envp[]){
	...
	if(Fg_setParameter(fg,FG_LINETRIGGERINSRC,&nTrgInSrc,nCamPort)			<0)		{ status = ErrorMessageWait(fg); return status;};
	if(Fg_setParameter(fg,FG_LINETRIGGERINPOLARITY,&nTrgInPolarity,nCamPort)<0)		{ status = ErrorMessageWait(fg); return status;};
	if(Fg_setParameter(fg,FG_LINE_DOWNSCALE,&nDownscale,nCamPort)			<0)		{ status = ErrorMessageWait(fg); return status;};

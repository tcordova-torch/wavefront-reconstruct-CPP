...
int main(int argc, char* argv[], char* envp[]){
	...
	int nTriggerMode		= GRABBER_CONTROLLED;
	int nExposureInMicroSec	= 10000;
	double dFramesPerSecond	= 10.0;
	int nExsyncOn = 1;
	...
	if(Fg_setParameter(fg,FG_FRAMESPERSEC,&dFramesPerSecond,nCamPort)<0){ status = ErrorMessageWait(fg); return status;}
	if(Fg_setParameter(fg,FG_EXSYNCON,&nExsyncOn,nCamPort)<0)			{ status = ErrorMessageWait(fg); return status;}

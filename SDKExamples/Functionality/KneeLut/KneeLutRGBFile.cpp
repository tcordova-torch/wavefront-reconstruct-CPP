...
#define FILENAME	"anOriginalKneeLutFilenameRgb.lut"
...
int main(int argc, char* argv[], char* envp[]){
	...
	KneeLookupTable LutR,LutG,LutB,getLut;

	LutR.id = 0;
	LutR.nrOfElements = 1025;
	LutR.value = new double[1025];
	LutR.format = FG_GRAY;
	LutG.id = 1;
	LutG.nrOfElements = 1025;
	LutG.value = new double[1025];
	LutG.format = FG_GRAY;
	LutB.id = 2;
	LutB.nrOfElements = 1025;
	LutB.value = new double[1025];
	LutB.format = FG_GRAY;


	for(i=0; i<(int)LutR.nrOfElements;i++)
	{
	...
	if(Fg_setParameter(fg, FG_KNEE_LUT,&LutR,nCamPort)<0){
		status = ErrorMessageWait(fg);
		return status;
	}
	if(Fg_setParameter(fg, FG_KNEE_LUT,&LutG,nCamPort)<0){
		status = ErrorMessageWait(fg);
		return status;
	}
	if(Fg_setParameter(fg, FG_KNEE_LUT,&LutB,nCamPort)<0){
		status = ErrorMessageWait(fg);
		return status;
	}

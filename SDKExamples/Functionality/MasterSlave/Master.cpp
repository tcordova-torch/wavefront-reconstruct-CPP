...
// File:	Master.cpp
//
// Example for using Master/Slave Mode This project demonstrates the master.
// 
// The example loads an applet onto the frame grabber in master mode and grabs
// images in Standard acquisition mode on port A. The corresponding slave
// example loads the applet in slave mode and grabs on port B.
// 
...
int main(int argc, char* argv[], char* envp[]){
	...
	// Initialize the frame grabber as a master
	int slaveMode = 0;
	if((fg = Fg_InitEx(dllName,nBoard, slaveMode)) == NULL) {
		status = ErrorMessageWait(fg);
		return status;
	}
	fprintf(stdout,"Init master frame grabber ok\n");

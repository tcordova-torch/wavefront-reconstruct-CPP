...
int main(int argc, char* argv[], char* envp[]){
	...
	dma_mem *pMem0;
	if((pMem0 = Fg_AllocMemHead(fg,totalBufSize,nr_of_buffer)) == NULL){
		status = ErrorMessageWait(fg);
		return status;
	} else {
	...
	unsigned char** buffer = (unsigned char**)malloc(nr_of_buffer*sizeof(unsigned char*));
	for(i=0;i<nr_of_buffer;i++){
		buffer[i] = (unsigned char*)malloc((size_t) width*height*bytesPerPixel);
		Fg_AddMem(fg,buffer[i],(size_t) width*height*bytesPerPixel,i,pMem0);
	}

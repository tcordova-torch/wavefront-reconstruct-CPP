...
int main(int argc, char* argv[], char* envp[]){
	...
	void *pMem0;
	if((pMem0 = Fg_AllocMem(fg,totalBufSize,nr_of_buffer,nCamPort)) == NULL){
		status = ErrorMessageWait(fg);
		return status;
	} else {
	...
	if((Fg_Acquire(fg,nCamPort,GRAB_INFINITE)) < 0){
		status = ErrorMessageWait(fg);
		return status;
	}
	// ====================================================
	...
	while((lastPicNr = Fg_getLastPicNumberBlocking(fg,lastPicNr+1,nCamPort,10))< MaxPics) {
		if(lastPicNr <0){
			status = ErrorMessageWait(fg);
		...
		::DrawBuffer(nId,Fg_getImagePtr(fg,lastPicNr,0),(int)lastPicNr,"");
	}

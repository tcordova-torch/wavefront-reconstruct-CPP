...
uint64_t getTimestampOfFirstNotifiedEvent( const uint64_t events, const struct fg_event_info * eventInfo)
{
	...
	// find out, which is the LSB at the bit mask in order to grab the according timestamp
...
int main(int argc, char* argv[], char* envp[]){
  ...
  if (!((boardType ==  PN_MICROENABLE4VD4CL )||
	...
	while (eventScanMask){
		...
		if(eventName){
			...
			if (currentEventMask){
				fprintf(stdout,"event mask : 0x%016llx event name: %s \n", (unsigned long long) currentEventMask, eventName);
			}
		}
  ...
  const char* eventToTrigger = events_to_trigger[eventToTriggerIndex];
	...
	availableEvents = 0;
	availableEvents = availableEvents | Fg_getEventMask(fg, eventToTrigger);// use the camera-link event
	if(availableEvents){
		...
		// 3. step: use the synchronous wait function and a timeout of 10 s
		fprintf(stdout,"Waiting for events \n");
		int maxNoOfEvents = 300; // terminate after 300 events
		int eventsWaited = 0;
		int timeout = 10;// wait 10 seconds
		...
		fprintf(stdout,"Eventmask activated: %016llX\n", (unsigned long long) availableEvents);

		// wait for the events
		while (eventsWaited < maxNoOfEvents){
			uint64_t eventsReceived = Fg_eventWait(fg, availableEvents, timeout, FG_EVENT_DEFAULT_FLAGS,&eventInfo);
			if (eventsReceived != 0){
				showEvents(eventsReceived,&eventData,&eventInfo);
			}
			eventsWaited++; // always increment, even if timed out
		}
		...
		fprintf(stdout,"Eventmask de-activated: %016llX\n", (unsigned long long) availableEvents);
